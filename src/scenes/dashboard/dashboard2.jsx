...
const Dashboard2 = ({ visuals: allVisuals = [], isGridActive }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  console.log("Received visuals in Dashboard2:", allVisuals);

  // Safely divide visuals
  const { dashboard2 = [] } = divideVisuals(allVisuals) || {};
  console.log("Divided visuals for Dashboard2:", dashboard2);
 ...
 // Monitor visuals to dynamically update the groupName and logo
    useEffect(() => {
      if (allVisuals && allVisuals.length > 0) {
        ...
        const logoURL = `/assets/logo/inpplc_logo.jpg`;
        ...
        fetch(logoURL, { method: 'HEAD' })
          .then((response) => {
            if (response.ok) {
              ...
              setLogoPath('/assets/logo/inpplc_logo.jpg'); // Fallback to default logo if not found
            }
          })
          .catch(() => {
            setLogoPath('/assets/logo/inpplc_logo.jpg'); // Handle fetch errors
          });
  ...
  const kpis = useMemo(
    () => (dashboard2 || []).filter((visual) => visual.type === "KPI"),
    [dashboard2]
  );
  console.log("Filtered KPIs for Dashboard2:", kpis);

  const charts = useMemo(
    () => (dashboard2 || []).filter((visual) => visual.type !== "KPI"),
    [dashboard2]
  );
  console.log("Filtered Charts for Dashboard2:", charts);

  // Combine KPIs and Charts into currentVisuals
  const currentVisuals = useMemo(() => [...kpis, ...charts], [kpis, charts]);
  console.log("Combined visuals for Dashboard2:", currentVisuals);
 ...
 // Define filterable keywords to search for in dimension sentences
    ...
    console.log("Filtered visuals for Dashboard2:", filteredVisuals);
    ...
    >
      ...
      <Box
        sx={{
          ...
          alignItems: "center",
          background: `linear-gradient(45deg,rgb(218, 220, 231),rgb(93, 172, 201))`,
          padding: "3px",
          backgroundImage: `url(/assets/logo/img_dash1.jpg)`,
          borderRadius: "12px",
      ...
      >
        ...
        <img src={logoPath || "/assets/logo/inpplc_logo.jpg"} alt="Dashboard Logo" width="100" height="80" />
        <Header 
            title={groupName} 
