...
const Dashboard = ({ visuals: allVisuals = [], isGridActive }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  console.log("Received visuals in Dashboard1:", allVisuals);

  // Safely divide visuals
  const { dashboard1 = [] } = divideVisuals(allVisuals) || {};
  console.log("Divided visuals for Dashboard1:", dashboard1);
 ...
 // Monitor visuals to dynamically update the groupName and logo
    useEffect(() => {
      if (allVisuals && allVisuals.length > 0) {
        ...
        const firstGroupName = allVisuals[1]?.group_name?.toLowerCase().replace(/\s+/g, '_') || "default_group";
        setGroupName(firstGroupName);

        // Construct logo URL from the public folder
        const logoURL = `/assets/logo/logoINPPLC.png`;
        ...
        fetch(logoURL, { method: 'HEAD' })
          .then((response) => {
            if (response.ok) {
              ...
              setLogoPath('/assets/logo/logoINPPLC.png'); // Fallback to default logo if not found
            }
          })
          .catch(() => {
            setLogoPath('/assets/logo/logoINPPLC.png'); // Handle fetch errors
          });
  ...
  const kpis = useMemo(
    () => (dashboard1 || []).filter((visual) => visual.type === "KPI"),
    [dashboard1]
  );
  console.log("Filtered KPIs for Dashboard1:", kpis);

  const charts = useMemo(
    () => (dashboard1 || []).filter((visual) => visual.type !== "KPI"),
    [dashboard1]
  );
  console.log("Filtered Charts for Dashboard1:", charts);

  // Combine KPIs and Charts into currentVisuals
  const currentVisuals = useMemo(() => [...kpis, ...charts], [kpis, charts]);
  console.log("Combined visuals for Dashboard1:", currentVisuals);
 ...
 // Define filterable keywords to search for in dimension sentences
    ...
    console.log("Filtered visuals for Dashboard1:", filteredVisuals);
    ...
    >
      ...
      <Box
        sx={{
          ...
          alignItems: "center",
          background: `linear-gradient(45deg,rgb(218, 220, 231),rgb(93, 145, 201))`,
          padding: "3px",
          borderRadius: "12px",
      ...
      >
        <Box display="flex" alignItems="center" gap="30px" >
          <img src={logoPath} alt="Dashboard Logo" width="90" height="100" />
          <Header 
            title={groupName} 
        ...
        <Box display="flex" alignItems="center" gap="15px">
          {dashboard1.length > 0 ? <FilterBar visuals={dashboard1} /> : <Typography>No filters available</Typography>}
          <Button
            sx={{
        ...
        >
          {filteredVisuals.length > 0 ? (
            filteredVisuals.map((visual, index) => (
              <div key={index} style={{ padding: "20px", backgroundColor: colors.primary[400], boxShadow: "2px 2px 2px 2px #ebebeb" }} onClick={() => handleOpenModal(visual)}>
      ...
      >
        <Box
          sx={{
            backgroundColor: theme.palette.mode === "dark" ? colors.primary[100] : colors.grey[400],
            padding: "20px",
            borderRadius: "8px",
            width: "50%",
          }}
